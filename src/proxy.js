...
class Proxy {
  constructor() {
    ...
    this._proxy.on('proxyRes', (proxyRes, req, res) =>
      this._onProxyRes(proxyRes, req, res)
    );
    this.updateConfig({});
    this.startProxy();
  }
  ...
  _isLocal(req, res) {
    if (
      ...
      (!isIp.test(req.headers.host) &&
        req.headers.host !==
          (process.env.MYHOST || 'localhost') + ':' + process.env.PORT &&
        req.headers.host !==
  ...
  _proxyingWeb(req, res) {
    ...
    routes
      ...
      .then(
        host => this._replaceServerUrl(host),
  ...
  _onError(err, req, res) {
    ...
    if (err.code === 'ECONNREFUSED')
      return this._routePage500(req, res, 'Error with ECONNREFUSED');
  ...
  _timeTrack(req, label) {
    ...
    if (req.headers['timetrack'] != '*' && req.headers['timetrack'] != req.url)
      return;
    if (!req.startTime) req.startTime = Date.now();
    if (!req.startTimeRandom)
      req.startTimeRandom = Math.floor(Math.random() * (1000000 + 1));

    console.log(
      `TimeTrack :: ${label} :: http://${req.headers.host}${req.url} ${
        req.startTime
      }-${req.startTimeRandom} ${Date.now() - req.startTime}`
    );
  }
  ...
  _setXHeaders(req) {
    ...
    req.headers['x-forwarded-proto'] =
      req.isSpdy || this._hasEncryptedConnection(req) ? 'https' : 'http';
  }
